...
#include <stdlib.h> // 동적 메모리 할당 함수(malloc, free)를 위한 헤더 파일
...
int maxHeight(BTNode *node);  // 이진 트리의 최대 높이를 계산하는 함수

BTNode *createBTNode(int item); // 새로운 이진 트리 노드를 생성하는 함수

BTNode *createTree();          // 이진 트리를 생성하는 함수
void push(Stack *stk, BTNode *node); // 스택에 노드를 추가하는 함수
BTNode* pop(Stack *stk);       // 스택에서 노드를 꺼내는 함수
...
// 이진 트리의 최대 높이를 계산하는 함수
int maxHeight(BTNode *node)
{
    // 기본 케이스: 노드가 NULL인 경우 높이는 -1
    if (node == NULL)
        return -1;
    ...
    // 왼쪽과 오른쪽 서브트리 중 더 높은 값에 1을 더해 반환
    // (현재 노드에서 자식 노드로 가는 링크를 포함하기 위해)
...
void push(Stack *stk, BTNode *node){
    ...
    if(stk->top == NULL){  // 스택이 비어있는 경우
        stk->top = temp;   // 새 노드를 스택 맨 위로 설정
        temp->next = NULL; // 다음 노드는 없음
    }
    else{  // 스택에 노드가 있는 경우
        temp->next = stk->top;  // 새 노드의 다음을 현재 스택 맨 위로 설정
        stk->top = temp;        // 스택 맨 위를 새 노드로 업데이트
    }
}
