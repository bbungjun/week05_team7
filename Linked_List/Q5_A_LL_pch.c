...
void frontBackSplitLinkedList(LinkedList *ll, LinkedList *resultFrontList, LinkedList *resultBackList)
{
	...
	while (current_node && idx < fixsize)
	{
		if(idx > fixsize / 2){
			insertNode(resultBackList, resultBackList->size, current_node->item);
		}else{
			insertNode(resultFrontList, resultFrontList->size, current_node->item);
		}
		idx++;
